...
// Find the shared memory for this original memory id
$sharedMemory = null;
$currentIndex = -1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['download'])) {
    if (!handleSecureDownload($_POST['download'], __DIR__ . '/../uploads/')) {
        die("File not found.");
    }
}

// Prepare attachments (support both 'attachment' and 'attachments')
$allAttachments = [];
...
// --- Prepare main memory media (single slider for all media types) ---
$mainMedia = [];
$mainOthers = [];
...
// --- Prepare attachments media (single slider for all media types) ---
$attMedia = [];
$attOthers = [];
$displayedFiles = [];
foreach ($allAttachments as $attachment) {
...
<div class="flex min-h-screen pt-20">
  <!-- Sidebar: Shared Memories Navigation -->
                ...
                <?php foreach ($attMedia as $i => $media): ?>
                  <div class="att-media-slide" data-index="<?= $i ?>" style="<?= $i === 0 ? '' : 'display:none;' ?>">
                    <?php if ($media['type'] === 'image'): ?>
                      <img src="<?= $media['src'] ?>" alt="<?= htmlspecialchars($media['name']) ?>" class="gallery-img w-full max-w-lg h-auto max-h-[400px] object-contain rounded-xl shadow border-2 border-[#8B7EC8] bg-white mb-2" />
                    <?php elseif ($media['type'] === 'video'): ?>
                      <video src="<?= $media['src'] ?>" controls class="w-full max-w-lg h-auto max-h-[400px] rounded-xl shadow border-2 border-[#8B7EC8] bg-black mb-2" type="video/<?= pathinfo($media['file'], PATHINFO_EXTENSION) ?>"></video>
                    <?php elseif ($media['type'] === 'audio'): ?>
                      <audio src="<?= $media['src'] ?>" controls class="w-full max-w-lg mb-2" type="audio/<?= pathinfo($media['file'], PATHINFO_EXTENSION) ?>"></audio>
                    <?php endif; ?>
                    <form method="post" action="" target="_blank">
                      <input type="hidden" name="download" value="<?= htmlspecialchars($media['file']) ?>" />
                      <button type="submit" class="px-3 py-1 bg-[#8B7EC8] text-white rounded-md text-xs font-semibold hover:bg-[#6BCB77] transition flex items-center gap-2">
