...
foreach ($myMemories as $memory) {
    ...
    $mFriends = isset($memory['friends']) && is_array($memory['friends']) ? $memory['friends'] : [];
    foreach ($mFriends as $friendEmail) {
...
foreach ($emails as $i => $email) {
    if ($email === $currentUserEmail)
        $userTypes[$i] = 'current';
    elseif (in_array($email, $friendEmails))
        $userTypes[$i] = 'friend';
    else
        $userTypes[$i] = 'other';
}
...
foreach ($allUsers as $email => $user) {
    $friendGraph[$email] = array_map(function ($f) {
        return $f['email'];
    }, $user['friends'] ?? []);
}
...
<head>
    ...
    <style>
        ...
        .graph-svg {
            background: #fff;
            border-radius: 2rem;
            box-shadow: 0 6px 24px 0 #e9d5ff44;
        ...
        #map {
            ...
            box-shadow: 0 4px 16px 0 #f3e8ff66;
            z-index: 1;
        }
        ...
        .soft-card {
            ...
            border-radius: 2rem;
            box-shadow: 0 6px 24px 0 #e9d5ff33;
            border: 1px solid #f3e8ff;
        }
                ...
                <div class="soft-card p-6 mb-8">
                    ...
                    <div class="flex flex-col items-center gap-2 mb-4">
                        <div class="soft-toggle flex w-full gap-2 max-w-xs mx-auto p-1">
                            <button id="toggleNetworkBtn"
                                class="px-4 py-2 text-sm font-semibold w-full">Network</button>
                        ...
                        <div id="graphControls" class="soft-toggle flex w-full gap-2 mt-2 max-w-xs mx-auto p-1">
                            <button id="showFriendshipsBtn"
                                class="px-2 py-1 text-xs font-semibold w-full">Friendships</button>
                            <button id="showMemoriesBtn"
                                class="px-2 py-1 text-xs font-semibold w-full">Memories</button>
                            <button id="showBothBtn" class="px-2 py-1 text-xs font-semibold w-full">Both</button>
                        </div>
                    </div>
                    <div id="graphView">
                        <div
                            class="flex flex-wrap justify-center items-center gap-x-6 gap-y-2 mb-4 p-4 bg-[#f3e8ff] rounded-xl text-sm">
                            <span class="font-bold">Legend:</span>
                            <span class="flex items-center"><span class="w-3 h-3 rounded-full bg-pink-400 mr-2"></span>
                                You</span>
                            <span class="flex items-center"><span
                                    class="w-3 h-3 rounded-full bg-purple-400 mr-2"></span> Friend</span>
                            <span class="flex items-center"><span class="w-3 h-3 rounded-full bg-teal-400 mr-2"></span>
                                Suggested</span>
                            <span class="flex items-center"><span class="w-3 h-3 rounded-full bg-gray-300 mr-2"></span>
                                Other</span>
                            <span class="flex items-center"><svg width="20" height="10" class="mr-2">
                                    <line x1="0" y1="5" x2="20" y2="5" stroke="#4ade80" stroke-width="2"
                                        marker-end="url(#arrowhead-suggestion-legend)" />
                                </svg> Suggestion Path</span>
                            <span class="flex items-center"><svg width="20" height="10" class="mr-2">
                                    <line x1="0" y1="5" x2="20" y2="5" stroke="#f9a8d4" stroke-width="2.5"
                                        stroke-dasharray="7,7" />
                                </svg> Your Memory</span>
                        </div>
                    ...
                    <div id="suggestions-list" class="space-y-2">
                        ...
                        <?php else: ?>
                            <?php foreach ($graphData['suggestedFriends'] as $name): ?>
                                <div class="p-2 rounded-lg bg-[#ede9fe]">
                                    <p class="font-semibold text-sm text-[#7c3aed]"><?= htmlspecialchars($name) ?></p>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        function initMap() {
            ...
            const processLocation = async (index) => {
                ...
                // If it was cached, we don't need a heavy delay. Otherwise, delay 600ms to avoid 429
