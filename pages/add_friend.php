...
$usersData = loadUsers();
$currentUser = getCurrentUser($usersData, $current_user_email);
if (!$currentUser)
    exit('User not found.');
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$friendName || !$howYouMet || !$friendEmail) {
        ...
        if (isset($_FILES['profileImage']) && $_FILES['profileImage']['error'] === UPLOAD_ERR_OK) {
            $upload = handleFileUpload($_FILES['profileImage'], __DIR__ . '/../uploads/friends/', ['jpg', 'jpeg', 'png', 'gif', 'webp'], 'friend_');
            if ($upload['success']) {
                $img = $upload['path'];
            } else {
...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Kalam:wght@400;700&family=Quicksand:wght@400;500;700&display=swap"
        rel="stylesheet">
    <style>
        html,
        body {
            ...
            <form id="addFriendForm" class="grid lg:grid-cols-2 gap-8" method="POST" enctype="multipart/form-data">
                <div class="lg:order-1 order-2">
                    <div class="text-center">
                        ...
                        <div class="relative inline-block">
                            <div id="imagePreview"
                                class="w-32 h-32 sm:w-40 sm:h-40 bg-[#F5F3FB] rounded-full border-4 border-[#D1C7EB] border-dashed flex items-center justify-center cursor-pointer hover:bg-[#E8E3F5] group">
                                <div id="placeholderContent" class="text-center">
                                    <i
                                        class="fas fa-camera text-3xl text-[#A394D4] mx-auto mb-2 group-hover:text-[#7A6BB5] transition"></i>
                                    <p class="text-sm text-[#7A6BB5] font-medium">Add Photo</p>
                                </div>
                                <img id="selectedImage" class="w-full h-full object-cover rounded-full hidden"
                                    alt="Friend profile" />
                            </div>
                            <input type="file" id="profileImage" name="profileImage" accept="image/*" class="hidden" />
                            <button type="button" id="removeImageBtn"
                                class="absolute -top-2 -right-2 w-8 h-8 bg-[#E88B8B] text-white rounded-full hidden hover:bg-[#E47777] transition">
                ...
                <div class="lg:order-2 order-1 space-y-6">
                    <div>
                        ...
                        <input type="text" id="userSearchInput" name="userSearchInput"
                            class="w-full px-4 py-3 border border-[#D1C7EB] rounded-lg focus:ring-2 focus:ring-[#8B7EC8] focus:border-transparent text-[#2D2A3D] placeholder-[#6B6B7D]"
                    ...
                    <div>
                        ...
                        <input type="email" id="friendEmail" name="friendEmail"
                            class="w-full px-4 py-3 border border-[#D1C7EB] rounded-lg focus:ring-2 focus:ring-[#8B7EC8] focus:border-transparent text-[#2D2A3D] placeholder-[#6B6B7D]"
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <button type="button"
                                class="relationship-tag px-4 py-2 border-2 border-[#D1C7EB] rounded-lg text-[#6B6B7D] hover:border-[#8B7EC8] hover:text-[#8B7EC8] transition"
                                data-tag="Best Friend">
                            ...
                            <button type="button"
                                class="relationship-tag px-4 py-2 border-2 border-[#D1C7EB] rounded-lg text-[#6B6B7D] hover:border-[#8B7EC8] hover:text-[#8B7EC8] transition"
                                data-tag="Acquaintance">
                                <i class="fab fa-meetup mr-2"></i> Acquaintance
                            </button>
                        </div>
                        <input type="text" id="customTag" name="customTag"
                            class="w-full px-4 py-3 border border-[#D1C7EB] rounded-lg focus:ring-2 focus:ring-[#8B7EC8] focus:border-transparent text-[#2D2A3D] placeholder-[#6B6B7D]"
                ...
                <div class="lg:col-span-2 order-3">
                    ...
                    <textarea id="howYouMet" name="howYouMet" rows="4" required maxlength="500"
                        class="w-full px-4 py-3 border border-[#D1C7EB] rounded-lg focus:ring-2 focus:ring-[#8B7EC8] focus:border-transparent text-[#2D2A3D] placeholder-[#6B6B7D] resize-none"
                        placeholder="Share the story of how you met this friend... What made your first encounter special?"></textarea>
                    <div class="flex justify-between items-center mt-2">
                        <div id="storyError" class="text-[#E88B8B] text-sm hidden">Please share how you met this friend
                        </div>
                ...
                <div class="lg:col-span-2 order-4 flex flex-col sm:flex-row gap-4 pt-6 border-t border-[#E8E3F5]">
                    <button type="submit"
                        class="flex-1 bg-[#8B7EC8] text-white px-8 py-4 rounded-xl font-medium hover:bg-[#7A6BB5] transition shadow hover:scale-105">
                        Save Friend
                    </button>
                    <button type="button" id="resetFormBtn"
                        class="flex-1 bg-white text-[#8B7EC8] px-8 py-4 rounded-xl font-medium hover:bg-[#F5F3FB] transition shadow border border-[#D1C7EB]">
                        Reset Form
                    </button>
                </div>
            </form>
