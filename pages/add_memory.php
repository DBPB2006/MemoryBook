...
if ($currentUser && !empty($currentUser['friends'])) {
    foreach ($currentUser['friends'] as $f) {
        if (!empty($f['email']))
            $availableFriends[$f['email']] = $f['name'] . ' (' . $f['email'] . ')';
    }
}
        ...
        <form id="memoryForm" class="grid lg:grid-cols-2 gap-8" method="POST" enctype="multipart/form-data">
            <div class="order-2 lg:order-1">
                <div class="bg-white rounded-2xl p-6 shadow mb-6">
                    ...
                    <video id="liveVideoPreview" class="w-full rounded-lg mb-4 hidden" autoplay muted
                        playsinline></video>
                    <div id="uploadArea" class="border-2 border-dashed border-[#D1C7EB] rounded-xl p-8 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <label
                                class="bg-[#8B7EC8] text-white px-4 py-2 rounded-lg hover:bg-[#7A6BB5] cursor-pointer">
                                Choose File
                                <input type="file" id="memoryMediaInput" name="memory_media[]" accept="*/*"
                                    class="hidden" multiple>
                            </label>
                            <div id="recordButtonsContainer" class="flex flex-wrap justify-center gap-4 mt-4">
                                <button type="button" id="recordAudioBtn"
                                    class="px-4 py-2 bg-[#8B7EC8] text-white rounded-lg hover:bg-[#7A6BB5] flex items-center gap-2 disabled:opacity-50"><i
                                        class="fa fa-microphone"></i> Record Audio</button>
                                <button type="button" id="recordVideoBtn"
                                    class="px-4 py-2 bg-[#8B7EC8] text-white rounded-lg hover:bg-[#7A6BB5] flex items-center gap-2 disabled:opacity-50"><i
                                        class="fa fa-video"></i> Record Video</button>
                                <button type="button" id="stopRecordingBtn"
                                    class="px-4 py-2 bg-[#8B7EC8] text-white rounded-lg hover:bg-[#7A6BB5] flex items-center gap-2 disabled:opacity-50"
                                    disabled><i class="fa fa-stop-circle"></i> Stop</button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow">
                    ...
                    <div class="relative">
                        <input type="text" id="location" name="location"
                            class="w-full px-4 py-3 border border-[#E8E3F5] rounded-lg focus:ring-2 focus:ring-[#8B7EC8]"
                            placeholder="Where did this memory happen?" autocomplete="off">
                        <i class="fas fa-map-marker-alt absolute right-3 top-4 text-[#6B6B7D]"></i>
                        <div id="locationSuggestions"
                            class="absolute z-10 w-full bg-white border border-[#E8E3F5] rounded-lg shadow-lg mt-1 hidden max-h-48 overflow-y-auto">
                        </div>
                    </div>
                </div>
            </div>
            <div class="order-1 lg:order-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl p-6 shadow">
                    <label for="userSearchInput" class="block text-lg font-semibold text-[#2D2A3D] mb-3">Add
                        Friends</label>
                    ...
                    <input type="text" id="userSearchInput" placeholder="Type a name or email..."
                        class="w-full px-4 py-3 border border-[#E8E3F5] rounded-lg" autocomplete="off">
                    <div id="userSuggestions" class="space-y-2 mt-2 max-h-40 overflow-y-auto"></div>
                    <div id="selectedFriends" class="mt-4"></div>
                </div>
                <div class="bg-white rounded-2xl p-6 shadow">
                    <label for="description" class="block text-lg font-semibold text-[#2D2A3D] mb-3">Memory
                        Description</label>
                    <textarea id="description" name="description" rows="4"
                        class="w-full px-4 py-3 border border-[#E8E3F5] rounded-lg focus:ring-2 focus:ring-[#8B7EC8]"
                        placeholder="Tell the story of this memory..."></textarea>
                ...
                <div class="bg-white rounded-2xl p-6 shadow">
                    <label for="tags" class="block text-lg font-semibold text-[#2D2A3D] mb-3">Tags (comma
                        separated)</label>
                ...
                <div class="flex gap-4 pt-6">
                    <button type="submit" name="final_submit"
                        class="flex-1 bg-[#8B7EC8] text-white px-6 py-3 rounded-lg hover:bg-[#7A6BB5] shadow">Create
                        Memory</button>
                </div>
            </div>
        </form>
