...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['download_path'])) {
    if (!handleSecureDownload($_POST['download_path'], __DIR__ . '/../uploads/')) {
        die("File not found.");
    }
}

// Load users (manual filter instead of array_filter)
...
// Load current user's memories
$allMemories = loadMemories();
$userMemories = [];
...
// Load recently shared memories
$allShared = loadSharedMemories();
$recentShared = [];
...
// Handle memory sharing form submission
$success = false;
$error = '';
$validationErrors = [];
if ($selectedUser && isset($_POST['memory_title'], $_POST['message'], $_POST['date'])) {
    ...
    // Handle optional file uploads using handleFileUpload
    if (isset($_FILES['attachments']) && is_array($_FILES['attachments']['name'])) {
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            if ($_FILES['attachments']['error'][$i] === UPLOAD_ERR_OK) {
                ...
                $upload = handleFileUpload($file, __DIR__ . '/../uploads/', ['jpg','jpeg','png','gif','webp','mp4','mov','avi','mp3','wav','ogg','m4a'], 'shared_');
                if ($upload['success']) {
                    $attachments[] = $upload['path'];
                } else {
                    $validationErrors['attachments'] = 'Failed to upload one or more files.';
                }
            }
        }
    }
        ...
        <ul class="space-y-4">
          <?php foreach ($recentShared as $mem): ?>
                ...
                <?php if (!empty($mem['attachments'])): ?>
                  <div class="media-gallery mt-2" id="<?= $galleryId ?>">
                    <?php foreach ($mem['attachments'] as $idx => $attachment):
                      ...
                      $downloadPath = (strpos($attachment, '../') === 0) ? substr($attachment, 3) : $attachment;
                    ...
                    ?>
                      <div class="media-slide" data-index="<?= $idx ?>" style="<?= $idx === 0 ? '' : 'display:none;' ?>">
                        ...
                        <?php elseif ($type === 'video'): ?>
                          <video src="<?= htmlspecialchars($attachment) ?>" controls class="max-h-32 rounded-lg mb-2" type="video/<?= pathinfo($attachment, PATHINFO_EXTENSION) ?>"></video>
                        <?php elseif ($type === 'audio'): ?>
                          <audio src="<?= htmlspecialchars($attachment) ?>" controls class="mb-2" type="audio/<?= pathinfo($attachment, PATHINFO_EXTENSION) ?>"></audio>
                        <?php else: ?>
                        ...
                        <form method="post" action="" class="download-form" style="display:inline;">
                          <input type="hidden" name="download_path" value="<?= htmlspecialchars($downloadPath) ?>">
                          <button type="submit" onclick="event.stopPropagation();" class="px-2 py-1 bg-[#8B7EC8] text-white rounded text-xs font-semibold hover:bg-[#6BCB77] transition flex items-center gap-2">
                    ...
                    <div class="flex gap-2 mt-2">
                      <button type="button" onclick="event.stopPropagation();" class="prev-btn px-2 py-1 bg-[#ede9fe] text-[#7c3aed] rounded font-semibold">Prev</button>
                      <button type="button" onclick="event.stopPropagation();" class="next-btn px-2 py-1 bg-[#ede9fe] text-[#7c3aed] rounded font-semibold">Next</button>
                    </div>
                  </div>
                <?php endif; ?>
              </div>
            ...
            document.addEventListener('DOMContentLoaded',function(){
                ...
                const prev = gallery.querySelector('.prev-btn');
                const next = gallery.querySelector('.next-btn');
                let cur = 0;
                function show(i){ slides.forEach((s,idx)=> s.style.display=(i===idx)?'':'none'); }
                if(prev) prev.onclick = (e) => { e.stopPropagation(); cur=(cur-1+slides.length)%slides.length; show(cur); };
                if(next) next.onclick = (e) => { e.stopPropagation(); cur=(cur+1)%slides.length; show(cur); };
            });
            </script>
          <?php endforeach; ?>
        </ul>
