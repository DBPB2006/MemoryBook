...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$firstName || !$lastName || !$email || !$password || !$confirmPassword) {
        ...
        if (isset($_FILES['profilePic']) && $_FILES['profilePic']['error'] === UPLOAD_ERR_OK) {
            $upload = handleFileUpload($_FILES['profilePic'], __DIR__ . '/../uploads/', ['jpg', 'jpeg', 'png', 'gif', 'webp'], 'avatar_');
            if ($upload['success']) {
                $profilePicPath = $upload['path'];
            } else {
                $signup_error = 'Failed to upload profile picture. Ensure the file is an image and under 2MB.';
            }
        } elseif (isset($_FILES['profilePic']) && $_FILES['profilePic']['error'] !== UPLOAD_ERR_NO_FILE) {
            $signup_error = 'Profile picture upload error code: ' . $_FILES['profilePic']['error'];
        }
        if (!$signup_error) {
            $details = [
                'username' => $username,
    ...
    <div class="w-full max-w-md">
        ...
        <div class="signup-container p-8">
            ...
            <form id="signupForm" class="space-y-6" method="POST" enctype="multipart/form-data" novalidate>
                ...
                <div>
                    <label class="block text-sm font-medium text-[#2D2A3D] mb-4">Profile Picture (optional)</label>
                    <div class="flex flex-col items-center space-y-4">
                        <div id="imagePreviewContainer" class="relative group">
                            <div class="relative w-24 h-24 rounded-full border-4 border-[#8B7EC8]/20 bg-gray-50 flex items-center justify-center overflow-hidden transition-all duration-300 group-hover:border-[#8B7EC8]/40">
                                <i id="userIcon" class="fas fa-user text-3xl text-gray-300"></i>
                                <img id="imagePreview" src="" alt="Profile Preview" class="absolute inset-0 hidden w-full h-full object-cover z-10 bg-white" style="display: none;">
                            </div>
                            <button type="button" id="removePhoto" class="hidden absolute -top-1 -right-1 bg-red-500 text-white w-6 h-6 rounded-full text-xs items-center justify-center hover:bg-red-600 transition-colors shadow-sm z-20">
                                <i class="fas fa-times"></i>
                            </button>
                        </div>
                        <div class="flex items-center justify-center w-full">
                            <label for="profilePic" class="cursor-pointer flex items-center space-x-2 px-6 py-2.5 rounded-xl border-2 border-[#8B7EC8] border-dashed text-[#8B7EC8] hover:bg-[#8B7EC8]/5 transition-all duration-200">
                                <i class="fas fa-camera"></i>
                                <span class="font-medium text-sm">Select Photo</span>
                            </label>
                            <input type="file" id="profilePic" name="profilePic" accept="image/*" class="hidden" />
                        </div>
                        <p class="text-xs text-[#6B6B7D]">Max size 2MB. JPG, PNG or WebP</p>
                    </div>
                </div>
