...
// Create and populate the MemoryList for the current user
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_memory']) && isset($_POST['id'])) {
    ...
    if ($deleteId) {
        // Find the node to delete to determine navigation before removing it
        $nodeToDelete = $memoryList->find($deleteId);
        $redirectId = null;

        if ($nodeToDelete) {
            // Determine redirect target: next, then previous, then null
        ...
        // Filter out the deleted memory from the master list
        $newMemories = [];
        foreach ($allMemories as $m) {
        ...
        // Save the updated master list back to the JSON file
        ...
        if ($redirectId) {
            ...
            // If no other memories are left, go to the main dashboard
...
// Find the current memory using the linked list
        ...
        <div class="flex flex-col gap-4 items-center max-h-[60vh] overflow-y-auto scrollbar-hide">
            <?php
                ...
                while ($node):
                    ...
                    // Determine state of the node for styling and navigation
                    $isActive = ($node === $currentNode);
                    $isNavigable = ($node === $currentNode->prev || $node === $currentNode->next);

                    // Determine href and classes based on state
                                ...
                                <?php foreach ($mediaList as $idx => $file): ?>
                                    ...
                                    <?php
                                        ...
                                        <?php elseif (strpos($type, 'video/') === 0): ?>
                                            <video src="../<?= htmlspecialchars($file['url']) ?>" controls class="w-full max-w-lg h-auto max-h-[400px] rounded-xl shadow border-2 border-[#8B7EC8] bg-black mb-2" type="<?= htmlspecialchars($file['type']) ?>"></video>
                                        <?php elseif (strpos($type, 'audio/') === 0): ?>
                                            <audio src="../<?= htmlspecialchars($file['url']) ?>" controls class="w-full max-w-lg mb-2" type="<?= htmlspecialchars($file['type']) ?>"></audio>
                                    <?php else: ?>
                                    ...
                                    <?php endif; ?>
                                        <?php
                                        // Robust download path logic as in open_capsule.php
