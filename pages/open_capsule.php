...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderMedia(index) {
        ...
        if (mediaEl) {
            ...
            // Add download button if unlocked
            const downloadForm = document.createElement('form');
            downloadForm.method = 'post';
            downloadForm.className = 'absolute bottom-4 right-4 z-20';
            const cleanPath = item.path.startsWith('uploads/') ? item.path : (item.path.includes('uploads/') ? item.path.substring(item.path.indexOf('uploads/')) : item.path);
            downloadForm.innerHTML = `
                <input type="hidden" name="download_path" value="${cleanPath}">
                <button type="submit" class="bg-[#8B7EC8] text-white px-4 py-2 rounded-lg shadow hover:bg-[#7A6BB5] transition flex items-center gap-2">
                    <i class="fas fa-download"></i> Download Media
                </button>
            `;
            mediaContentEl.appendChild(downloadForm);
        }
    }
